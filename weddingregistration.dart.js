...
$$.WeddingState = {"":
 ["stateNum=", "completionState=", "dietaryState=", "restrictionState=", "stateMap?"],
 "super": "Object",
 WeddingState$0: function() {
  this.stateMap = $.HashMapImplementation$();
  var t1 = this.stateMap;
  $.indexSet(t1, this.stateNum, $.RegState$(0, 'StartScreen', 'Press [ENTER]', '', ''));
  this.stateNum = $.add(this.stateNum, 1);
  $.indexSet(t1, this.stateNum, $.RegState$(0, 'Intro', 'Welcome to the LePosey RSVP system.', '', ''));
  this.stateNum = $.add(this.stateNum, 1);
  $.indexSet(t1, this.stateNum, $.RegState$(0, 'Flavor01', 'Presumably you\'ve been invited to our wedding. Correct?', '', ''));
  this.stateNum = $.add(this.stateNum, 1);
  $.indexSet(t1, this.stateNum, $.RegState$(1, '#memoryInpt', 'To prove it, what is your fondest memory of the bride and/or groom?', '#Memory', ''));
  this.stateNum = $.add(this.stateNum, 1);
  $.indexSet(t1, this.stateNum, $.RegState$(2, '#nameInpt', 'OK, before we start, what is your name?', '#FirstName', '#LastName'));
  this.stateNum = $.add(this.stateNum, 1);
  $.indexSet(t1, this.stateNum, $.RegState$(3, '#comingInpt', 'Are you coming to the wedding?', '#NoComing', ''));
  this.stateNum = $.add(this.stateNum, 1);
  $.indexSet(t1, this.stateNum, $.RegState$(3, '#plus1Inpt', 'Are you bringing a plus 1?', '#NoGuest', ''));
  this.stateNum = $.add(this.stateNum, 1);
  $.indexSet(t1, this.stateNum, $.RegState$(2, '#plus1nameInpt', 'What is the name of your guest?', '#Plus1FirstName', '#Plus1LastName'));
  this.stateNum = $.add(this.stateNum, 1);
  this.dietaryState = this.stateNum;
  $.indexSet(t1, this.stateNum, $.RegState$(3, '#mainFoodInpt', 'Pick your meal type!', '#NoGuest', ''));
  this.stateNum = $.add(this.stateNum, 1);
  $.indexSet(t1, this.stateNum, $.RegState$(3, '#guestFoodInpt', 'What\'s your guest\'s meal type?', '', ''));
  this.stateNum = $.add(this.stateNum, 1);
  this.restrictionState = this.stateNum;
  $.indexSet(t1, this.stateNum, $.RegState$(1, '#restrictionInpt', 'Do you have any dietary restrictions?', '', ''));
  this.stateNum = $.add(this.stateNum, 1);
  this.completionState = this.stateNum;
  $.indexSet(t1, this.stateNum, $.RegState$(4, '#submitInpt', 'Thank you for your RSVP. Click \'Submit\' to finish.', '', ''));
  this.stateNum = $.add(this.stateNum, 1);
}
};

$$.RegState = {"":
 ["sType", "name?", "textValue?", "inputName1", "inputName2"],
 "super": "Object",
 ValidateInput$0: function() {
  var t1 = this.inputName1;
  if ($.eqB(t1, ''))
    return 1;
  switch (this.sType) {
    case 1:
      if ($.eqB($.trim($.query(t1).get$value()), ''))
        return 0;
      break;
    case 2:
      var fname = $.query(t1);
      var lname = $.query(this.inputName2);
      if ($.eqB(fname.get$value(), '') || $.eqB(lname.get$value(), ''))
        return 0;
      break;
    case 3:
      if ($.eqB($.query(t1).get$checked(), true))
        return -1;
      break;
  }
  return 1;
...
$.writeChar = function(t) {
  ...
  if (typeof t1 !== 'string' && (typeof t1 !== 'object' || t1 === null || t1.constructor !== Array && !t1.is$JavaScriptIndexingBehavior()))
    return $.writeChar$bailout(1, t1, 0, 0);
  var t3 = $.counter;
  if (t3 !== (t3 | 0))
    throw $.iae(t3);
  if (t3 < 0 || t3 >= t1.length)
  ...
  t1 = $.writingCounter;
  if (typeof t1 !== 'number')
    return $.writeChar$bailout(2, t1, s, 0);
  t3 = $.get$length(s);
  if (typeof t3 !== 'number')
    return $.writeChar$bailout(3, t1, t3, s);
  if (t1 < t3) {
    var textElement = $.query('#text');
    t1 = $.writingCounter;
    if (typeof t1 !== 'number')
      return $.writeChar$bailout(4, textElement, t1, s);
    textElement.set$text($.substring$2(s, 0, t1 + 1));
    t3 = $.writingCounter;
    if (typeof t3 !== 'number')
      return $.writeChar$bailout(5, t3, 0, 0);
    $.writingCounter = t3 + 1;
...
$.showCurrentInput = function() {
  var inpt = $.query($.index($.state.get$stateMap(), $.counter).get$name());
  if (!(inpt == null))
    inpt.set$hidden(false);
  if ($.eqB($.index($.state.get$stateMap(), $.counter).get$name(), '#submitInpt'))
    if ($.eqB($.get$length(inpt.get$elements()), 0)) {
      var sub = $._Elements_InputElement('submit');
      sub.set$name('submit');
      sub.set$value('Submit');
      $.add$1(inpt.get$elements(), sub);
    }
};
...
$.main = function() {
  ...
  $.writingCounter = 0;
  $.state = $.WeddingState$();
  $.indexSet($.state.get$stateMap(), $.state.get$stateNum(), $.RegState$(0, 'StartScreen', 'Press [ENTER]', '', ''));
  var t1 = $.state;
  t1.set$stateNum($.add(t1.get$stateNum(), 1));
  $.indexSet($.state.get$stateMap(), $.state.get$stateNum(), $.RegState$(0, 'Intro', 'Welcome to the LePosey RSVP system.', '', ''));
  t1 = $.state;
  t1.set$stateNum($.add(t1.get$stateNum(), 1));
  $.indexSet($.state.get$stateMap(), $.state.get$stateNum(), $.RegState$(0, 'Flavor01', 'Presumably you\'ve been invited to our wedding. Correct?', '', ''));
  t1 = $.state;
  t1.set$stateNum($.add(t1.get$stateNum(), 1));
  $.indexSet($.state.get$stateMap(), $.state.get$stateNum(), $.RegState$(1, '#memoryInpt', 'To prove it, what is your fondest memory of the bride and/or groom?', '#Memory', ''));
  t1 = $.state;
  t1.set$stateNum($.add(t1.get$stateNum(), 1));
  $.indexSet($.state.get$stateMap(), $.state.get$stateNum(), $.RegState$(2, '#nameInpt', 'OK, before we start, what is your name?', '#FirstName', '#LastName'));
  t1 = $.state;
  t1.set$stateNum($.add(t1.get$stateNum(), 1));
  $.indexSet($.state.get$stateMap(), $.state.get$stateNum(), $.RegState$(3, '#comingInpt', 'Are you coming to the wedding?', '#NoComing', ''));
  t1 = $.state;
  t1.set$stateNum($.add(t1.get$stateNum(), 1));
  $.indexSet($.state.get$stateMap(), $.state.get$stateNum(), $.RegState$(3, '#plus1Inpt', 'Are you bringing a plus 1?', '#NoGuest', ''));
  t1 = $.state;
  t1.set$stateNum($.add(t1.get$stateNum(), 1));
  $.indexSet($.state.get$stateMap(), $.state.get$stateNum(), $.RegState$(2, '#plus1nameInpt', 'What is the name of your guest?', '#Plus1FirstName', '#Plus1LastName'));
  t1 = $.state;
  t1.set$stateNum($.add(t1.get$stateNum(), 1));
  t1 = $.state.get$stateNum();
  $.state.set$dietaryState(t1);
  $.indexSet($.state.get$stateMap(), $.state.get$stateNum(), $.RegState$(3, '#mainFoodInpt', 'Pick your meal type!', '#NoGuest', ''));
  t1 = $.state;
  t1.set$stateNum($.add(t1.get$stateNum(), 1));
  $.indexSet($.state.get$stateMap(), $.state.get$stateNum(), $.RegState$(3, '#guestFoodInpt', 'What\'s your guest\'s meal type?', '', ''));
  t1 = $.state;
  t1.set$stateNum($.add(t1.get$stateNum(), 1));
  t1 = $.state.get$stateNum();
  $.state.set$restrictionState(t1);
  $.indexSet($.state.get$stateMap(), $.state.get$stateNum(), $.RegState$(1, '#restrictionInpt', 'Do you have any dietary restrictions?', '', ''));
  t1 = $.state;
  t1.set$stateNum($.add(t1.get$stateNum(), 1));
  t1 = $.state.get$stateNum();
  $.state.set$completionState(t1);
  $.indexSet($.state.get$stateMap(), $.state.get$stateNum(), $.RegState$(4, '#submitInpt', 'Thank you for your RSVP. Click \'Submit\' to finish.', '', ''));
  t1 = $.state;
  t1.set$stateNum($.add(t1.get$stateNum(), 1));
...
$.WeddingState$ = function() {
  var t1 = new $.WeddingState(0, 0, 0, 0, null);
  t1.WeddingState$0();
  return t1;
};
...
$.onKeyDown = function(event$) {
  ...
  if ($.eqB(event$.get$keyCode(), 13)) {
    if ($.eqB($.isWriting, true)) {
      $.isWriting = false;
      $.writingCounter = 2000;
      textElement.set$text($.index($.state.get$stateMap(), $.counter).get$textValue());
      $.showCurrentInput();
      return;
    }
    switch ($.index($.state.get$stateMap(), $.counter).ValidateInput$0()) {
      case -1:
        if ($.eqB($.index($.state.get$stateMap(), $.counter).get$name(), '#comingInpt'))
          $.counter = $.state.get$completionState();
        else if ($.eqB($.index($.state.get$stateMap(), $.counter).get$name(), '#plus1Inpt'))
          $.counter = $.state.get$dietaryState();
        else if ($.eqB($.index($.state.get$stateMap(), $.counter).get$name(), '#mainFoodInpt'))
          $.counter = $.state.get$restrictionState();
        break;
      case 0:
        return;
      default:
    ...
    if ($.ltB($.counter, $.state.get$stateNum())) {
      $.hideAllInput();
      if ($.eqB($.index($.state.get$stateMap(), $.counter).get$name(), 'Intro')) {
...
$.RegState$ = function(sType, name$, textValue, inputName1, inputName2) {
  return new $.RegState(sType, name$, textValue, inputName1, inputName2);
};
...
$.writeChar$bailout = function(state, env0, env1, env2) {
  ...
  switch (state) {
    ...
    case 3:
      state = 0;
    default:
      if (state === 5 || state === 4 || state === 0 && $.ltB(t1, t3))
        switch (state) {
          case 0:
            var textElement = $.query('#text');
            t1 = $.writingCounter;
...
//@ sourceMappingURL=WeddingRegistration.dart.js.map